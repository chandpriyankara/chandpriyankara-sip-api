...
public class GracefulStopTest extends SipServletTestCase {
	...
	public SipContext deployShootme(String webContextName) {
	...
	public SipContext deployConvergedApp() {
		SipStandardContext context = new SipStandardContext();
	...
	/**
	 * Make sure that the graceful stop guarantees the application is accepting subsequent requests
	 * but not new requests and that when no sessions are present the context is stopped automatically
	 */
	public void testContextGracefulShutdownNegativeValue() throws Exception {
		tomcat.startTomcat();
		SipContext sipContext = deployShootme(null);
	...
	/**
	 * Make sure that the graceful stop guarantees the application is stopped forcefully by using 0 as time to Wait
	 */
	...
	/**
	 * Make sure that the graceful stop guarantees the application is stopped forcefully after TIMEOUT
	 * and that the result is equivalent to zero value in our case
	 */
...
//		tomcat.getSipService().stopGracefully(-1);
	...
	/**
	 * Make sure that the graceful stop guarantees the application is accepting subsequent HTTP requests
	 * but not new HTTP requests and that when no sessions are present the context is stopped automatically
	 */
	public void testConvergedContextGracefulShutdownNegativeValue() throws Exception {
		String CLICK2DIAL_URL = "http://" + System.getProperty("org.mobicents.testsuite.testhostaddr") + ":8080/click2call/call";
		String CLICK2DIAL_PARAMS = "?from=sip:from@" + System.getProperty("org.mobicents.testsuite.testhostaddr") + ":5056&to=sip:to@" + System.getProperty("org.mobicents.testsuite.testhostaddr") + ":5057&notification=false";
		tomcat.startTomcat();
		SipContext sipContext = deployConvergedApp();
		

		senderProtocolObjects =new ProtocolObjects(
				"sender", "gov.nist", TRANSPORT, AUTODIALOG, null, null, null);
		
		sender = new TestSipListener(5080, 5057, senderProtocolObjects, true);
		SipProvider senderProvider = sender.createProvider();			
